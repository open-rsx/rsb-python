...
class Coverage(Command):
    ...
    def finalize_options(self):
        ...
        if self.spread == None and rsb.haveSpread():
            ...
            if self.spread == None:
                print('WARNING: no spread daemon found. '
                      'Make sure that one is running before '
                      'starting the coverage report')

    def run(self):

        spread = nullContext()
        if self.spread:
            spread = CommandStarter([self.spread,
                                     '-n', 'localhost',
                                     '-c', 'test/spread.conf'])
...
class Test(setuptools.command.test.test):
    ...
    user_options = setuptools.command.test.test.user_options \
        + [('spread=', 'd',
            'Spread executable to use'),
           ('spreadport=', 'p',
            'Port the spread daemon should use'),
           ('socketport=', None,
            'Port which should be used by socket transport')]
...
version, commit = defineProjectVersion('0.13')
